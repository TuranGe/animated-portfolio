<script>
  ...
  let _gsapReady;
  let _resolveGsap;
  _gsapReady = new Promise(r => { _resolveGsap = r; });

  function toggleTheme() {
    theme.update(t => {
      const next = t === 'dark' ? 'light' : 'dark';
      document.documentElement.setAttribute('data-theme', next);
      if (typeof localStorage !== 'undefined') localStorage.setItem('kuon-theme', next);
      return next;
    });
  }

  function goSection(i) { mobileOpen = false; onNavigate(i); }

  export async function hideSidebar() {
    await _gsapReady;
    if (!gsap || !sideNavEl) return;
    await gsap.to(sideNavEl, { x: -60, opacity: 0, duration: 0.45, ease: 'expo.in' });
  }

  export async function showSidebarAnim() {
    await _gsapReady;
    if (!gsap || !sideNavEl) return;
    gsap.fromTo(sideNavEl, { x: -60, opacity: 0 }, { x: 0, opacity: 1, duration: 0.55, ease: 'expo.out' });
  }

  export async function showBackAnim() {
    await _gsapReady;
    if (!gsap || !backBtnEl) return;
    gsap.fromTo(backBtnEl, { x: -22, opacity: 0 }, { x: 0, opacity: 1, duration: 0.9, ease: 'expo.out', delay: 0.4 });
  }

  export async function hideBackAnim() {
    await _gsapReady;
    if (!gsap || !backBtnEl) return;
    await gsap.to(backBtnEl, { x: -22, opacity: 0, duration: 0.9, ease: 'expo.in' });
  }
  ...
  onMount(async () => {
    ...
    function onWindowScroll() { applyScrolled(window.scrollY); }
    window.addEventListener('scroll', onWindowScroll, { passive: true });

    // Also watch any .page-scroll container (detail pages)
    const scrollContainers = document.querySelectorAll('.page-scroll');
    function onContainerScroll(e) { applyScrolled(e.currentTarget.scrollTop); }
    scrollContainers.forEach(el => el.addEventListener('scroll', onContainerScroll, { passive: true }));
...
<nav id="top-nav" bind:this={topNavEl}>
  <div style="display:flex;flex-direction:column;gap:2px;">
    <a href="/"
      style="font-family:'Playfair Display',serif;font-size:1.1rem;font-weight:600;
             letter-spacing:.12em;color:{subPage ? 'var(--fg)' : 'var(--fg)'};text-decoration:none;line-height:1.2;">
      PORTFOLIO
    </a>
    ...
    {#if onBack}
      <button
        ...
        style="display:flex;align-items:center;gap:8px;background:none;border:none;
               ...
               color:var(--fg2);"
      >
        <svg width="16" height="11" viewBox="0 0 16 11" fill="none" style="flex-shrink:0;">
          <path d="M15 5.5H1M1 5.5L5.5 1M1 5.5L5.5 10" stroke="currentColor" stroke-width="1.3"
                stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
        <span style="font-family:'JetBrains Mono',monospace;font-size:.58rem;letter-spacing:.18em;text-transform:uppercase;">
          BACK
        </span>
      </button>
    {/if}
  ...
  <div style="display:flex;align-items:center;gap:20px;">
    ...
    <button class="theme-toggle" on:click={toggleTheme} aria-label="Toggle theme" style="">
    ...
    {#if !subPage}
      <div class="desktop-links" bind:this={navLinksEl}>
        {#each sections as s, i}
          <button on:click={() => goSection(i)}
            style="font-family:'JetBrains Mono',monospace;font-size:.62rem;letter-spacing:.18em;
                   text-transform:uppercase;background:none;border:none;
                   transition:color .3s;color:{$currentSection === i ? 'var(--accent)' : 'var(--fg2)'};">
            {s.label}
          </button>
        {/each}
      </div>

      <button class="ham-btn" on:click={() => mobileOpen = !mobileOpen} aria-label="Menu" style="">
        <span style="display:block;width:20px;height:1.5px;background:var(--fg);transition:all .3s;
          {mobileOpen ? 'transform:rotate(45deg) translateY(5px);' : ''}"></span>
        <span style="display:block;width:20px;height:1.5px;background:var(--fg);transition:all .3s;
          margin:5px 0;{mobileOpen ? 'opacity:0;' : ''}"></span>
        <span style="display:block;width:20px;height:1.5px;background:var(--fg);transition:all .3s;
          {mobileOpen ? 'transform:rotate(-45deg) translateY(-5px);' : ''}"></span>
      </button>
    {/if}
...
{#if showSidebar}
  <nav id="side-nav" bind:this={sideNavEl}>
    {#each sections as s, i}
      <button class="side-nav-item" class:active={$currentSection === i}
        on:click={() => goSection(i)} aria-label={s.label}
...
<style>
  .desktop-links { display:none; gap:28px; }
  .ham-btn { display:flex; flex-direction:column; padding:4px; background:none; border:none; }
  @media (min-width:768px) {
    .desktop-links { display:flex; }
    .ham-btn       { display:none; }
  }

  .back-btn       { color: var(--fg2); transition: color .25s; }
