<script>
  ...
  async function openWork(slug, cardEl, solidColor, cardIdx) {
    ...
    if (!cardEl) { goto('/work/' + slug); return; }
    ...
    flipState.set({ rect: { left:btnRect.left, top:btnRect.top, width:btnRect.width, height:btnRect.height, padTop, padLeft }, solidColor, slug, cardIdx });
    ...
    if (innerRow) {
      ...
      rowCopy.querySelectorAll('h3,.row-title').forEach(el => { el.style.color = fg;  });
      rowCopy.querySelectorAll('.label')       .forEach(el => { el.style.color = fg2; });
    ...
    if (topNav) gsap.to(topNav, { y: -8, opacity: 0, duration: 0.35, ease: 'expo.in' });
    navEl?.hideSidebar();

    // Expand clone to fullscreen, simultaneously animate padding to match detail page
    gsap.to(rowWrapper, {
      paddingLeft: targetPadH, paddingRight: targetPadH,
  ...
  async function runReturnAnimation() {
    ...
    try {
      ...
      if (!raw) { _returnDone = false; return; }
      sessionStorage.removeItem('flipState');
      state = JSON.parse(raw);
    } catch(e) { _returnDone = false; return; }
    ...
    const liveCardIdx = typeof state.cardIdx === 'number' ? state.cardIdx : null;
    ...
    if (liveCardEl) {
      ...
      liveRect = { left: r.left, top: r.top, width: r.width, height: r.height, padTop: livePadTop, padLeft: livePadLeft };
      state.rect = liveRect;
    }
    ...
    if (existingClone && gsap && liveCardEl) {
      ...
      gsap.to(existingClone, {
        left: liveRect.left, top: liveRect.top,
        width: liveRect.width, height: liveRect.height,
        paddingTop: endPadTop,
        duration: 0.65, ease: 'expo.inOut',
        onComplete: () => { existingClone.remove(); window.__returnClone = null; }
      });
    }
    ...
    const topNav = document.getElementById('top-nav');
    if (topNav && gsap) {
      gsap.fromTo(topNav, { y: -60, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, ease: 'expo.out', delay: 0.4 });
    }
  }
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (rs !== null) { returnSection = parseInt(rs); sessionStorage.removeItem('returnSection'); }
    } catch(e) {}
    ...
    if (returnSection >= 0) {
      try { sessionStorage.removeItem('flipState'); } catch(e) {}
    }

    let hasFlip = false;
    try { hasFlip = !!sessionStorage.getItem('flipState'); } catch(e) {}
    ...
    if (hasFlip) {
      ...
      if (topNavEl) gsap.fromTo(topNavEl, { opacity:0, y:-60 }, { opacity:1, y:0, duration:0.85, ease:'expo.out', delay:0.1 });
      if (heroLine)   tl.fromTo(heroLine,   { scaleX:0 },       { scaleX:1,  duration:0.7, ease:'expo.out' });
      if (heroSub)    tl.fromTo(heroSub,    { y:30,opacity:0 }, { y:0,opacity:1, duration:0.7, ease:'expo.out' }, '-=0.4');
      if (heroTitle)  tl.fromTo(heroTitle,  { y:60,opacity:0 }, { y:0,opacity:1, duration:0.9, ease:'expo.out' }, '-=0.5');
      if (heroScroll) tl.fromTo(heroScroll, { opacity:0 },      { opacity:1, duration:0.5 }, '-=0.1');
    }

    // When user navigates to section 0 after returning, animate hero in
    const unsubSection = currentSection.subscribe(idx => {
      if (idx === 0 && heroTitle) {
        const titleVisible = parseFloat(getComputedStyle(heroTitle).opacity) > 0.1;
        if (!titleVisible) {
          const tl = gsap.timeline({ delay: 0.1 });
          if (heroLine)   tl.fromTo(heroLine,   { scaleX:0 },       { scaleX:1,  duration:0.7, ease:'expo.out' });
          if (heroSub)    tl.fromTo(heroSub,    { y:20,opacity:0 }, { y:0,opacity:1, duration:0.6, ease:'expo.out' }, '-=0.3');
          if (heroTitle)  tl.fromTo(heroTitle,  { y:40,opacity:0 }, { y:0,opacity:1, duration:0.8, ease:'expo.out' }, '-=0.4');
          if (heroScroll) tl.fromTo(heroScroll, { opacity:0 },      { opacity:1, duration:0.5 }, '-=0.1');
        }
      }
    });
    ...
    function onMouseMove(e) {
      ...
      gsap.to(heroSvg, { x: dx * 22, y: dy * 14, duration: 1.4, ease: 'power2.out' });
    }
...
<svelte:head><title>Portfolio — Creative Developer</title></svelte:head>
...
<div id="fp-container" style="height:100vh;overflow:hidden;position:relative;">
  <div id="fullpage" style="will-change:transform;transform:translateY({initY});">

    <!-- ══ SECTION 0 · HERO ══ -->
    <section class="fp-section" style="background:var(--bg);">
      <div style="position:absolute;inset:0;pointer-events:none;
        background-image:linear-gradient(var(--border) 1px,transparent 1px),linear-gradient(90deg,var(--border) 1px,transparent 1px);
        background-size:72px 72px;opacity:.5;"></div>
      <!-- Ambient glows -->
      <div style="position:absolute;width:600px;height:600px;border-radius:50%;top:-150px;right:-80px;background:radial-gradient(circle,var(--accent) 0%,transparent 65%);filter:blur(100px);opacity:.07;pointer-events:none;"></div>
      <div style="position:absolute;width:400px;height:400px;border-radius:50%;bottom:-100px;left:5%;background:radial-gradient(circle,var(--accent) 0%,transparent 65%);filter:blur(80px);opacity:.06;pointer-events:none;"></div>

      <div class="hero-grid" style="position:relative;z-index:10;width:100%;padding:0 clamp(24px,6vw,96px);
        ...
        <div style="position:relative;z-index:2;">
          ...
          <div bind:this={heroScroll} style="display:flex;align-items:center;gap:16px;opacity:0;">
            <span class="label" style="writing-mode:vertical-rl;color:var(--fg2);">SCROLL DOWN</span>
            <div style="width:1px;height:48px;background:var(--fg2);opacity:.4;"></div>
          </div>
        </div>

        <!-- RIGHT: SVG with parallax -->
        <div bind:this={heroSvg} class="hero-svg-wrap" style="display:flex;align-items:center;justify-content:center;will-change:transform;">
          <svg viewBox="0 0 480 480" width="100%" style="max-width:520px;" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="240" cy="240" r="220" stroke="var(--accent)" stroke-width=".6" opacity=".2"/>
            <circle cx="240" cy="240" r="170" stroke="var(--accent)" stroke-width=".8" stroke-dasharray="4 8" opacity=".3"/>
            <circle cx="240" cy="240" r="110" stroke="var(--accent)" stroke-width="1" opacity=".45"/>
            <line x1="240" y1="95"  x2="240" y2="385" stroke="var(--accent)" stroke-width=".6" opacity=".25"/>
            <line x1="95"  y1="240" x2="385" y2="240" stroke="var(--accent)" stroke-width=".6" opacity=".25"/>
            <line x1="157" y1="157" x2="323" y2="323" stroke="var(--accent)" stroke-width=".5" opacity=".15"/>
            <line x1="323" y1="157" x2="157" y2="323" stroke="var(--accent)" stroke-width=".5" opacity=".15"/>
            <line x1="240" y1="70"  x2="410" y2="240" stroke="var(--accent)" stroke-width="1.5" opacity=".55"/>
            <rect x="226" y="226" width="28" height="28" stroke="var(--accent)" stroke-width="1.1" opacity=".65" transform="rotate(45 240 240)"/>
            <circle cx="240" cy="240" r="40" fill="var(--accent)" opacity=".05"/>
            <circle cx="240" cy="240" r="6"  fill="var(--accent)" opacity=".7"/>
            <circle cx="240" cy="240" r="2"  fill="var(--fg)"    opacity="1"/>
            <text x="248" y="52"  font-family="JetBrains Mono,monospace" font-size="9" fill="var(--accent)" opacity=".4" letter-spacing="2">N 48°52</text>
            <text x="396" y="244" font-family="JetBrains Mono,monospace" font-size="9" fill="var(--accent)" opacity=".4" letter-spacing="2">E 02°21</text>
            <circle cx="186" cy="148" r="2.5" fill="var(--accent)" opacity=".45"/>
            <circle cx="240" cy="130" r="2"   fill="var(--accent)" opacity=".35"/>
            <circle cx="294" cy="148" r="2.5" fill="var(--accent)" opacity=".45"/>
            <circle cx="332" cy="240" r="2.5" fill="var(--accent)" opacity=".4"/>
          </svg>
        </div>
      </div>
    ...
    <!-- ══ SECTION 1 · WORK ══ -->
    <section class="fp-section" bind:this={workSectionEl}
      style="background:var(--bg);align-items:flex-start;padding-top:80px;">
      <!-- Ambient glows -->
      <div style="position:absolute;width:500px;height:500px;border-radius:50%;bottom:-80px;right:-60px;background:radial-gradient(circle,var(--accent) 0%,transparent 65%);filter:blur(90px);opacity:.07;pointer-events:none;"></div>
      <div style="position:absolute;width:300px;height:300px;border-radius:50%;top:10%;left:-50px;background:radial-gradient(circle,var(--accent) 0%,transparent 65%);filter:blur(70px);opacity:.05;pointer-events:none;"></div>
      <div style="width:100%;padding:0 clamp(24px,6vw,96px);overflow-y:auto;max-height:100vh;padding-bottom:40px;">
        <div style="max-width:1100px;margin:0 auto;">
          <div style="margin-bottom:40px;">
            <p class="label" style="margin-bottom:10px;">01 — SELECTED WORKS</p>
            <h2 style="font-family:'Playfair Display',serif;font-size:clamp(2rem,5vw,4rem);
          ...
          {#each works as w, i}
            ...
            >
              <div style="display:flex;align-items:center;gap:20px;">
                <span class="label" style="width:32px;flex-shrink:0;color:var(--fg2);">{w.num}</span>
                ...
                <div style="width:96px;height:64px;border-radius:6px;overflow:hidden;flex-shrink:0;
                  ...
                  <div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;">
                    <span style="font-size:1.4rem;opacity:.8;color:{w.icolor};">◆</span>
                  </div>
                </div>
                ...
                <div style="flex:1;">
                  ...
                  <p class="label" style="color:var(--fg2);">{w.cat} — {w.year}</p>
                </div>

                <span class="row-arrow" style="color:{w.icolor};font-size:1.4rem;flex-shrink:0;transition:transform .35s;">→</span>
              </div>

              <div style="height:1px;margin-top:28px;position:relative;overflow:hidden;background:var(--border);">
                <div class="hover-line"
                  style="position:absolute;inset-y:0;left:0;width:0;background:{w.solidColor};transition:width .65s ease;"></div>
              </div>
            </button>
          {/each}
        </div>
    ...
    <section class="fp-section" style="background:var(--bg);">
      <!-- Ambient glows -->
      <div style="position:absolute;width:550px;height:550px;border-radius:50%;top:-120px;left:-100px;background:radial-gradient(circle,var(--accent) 0%,transparent 65%);filter:blur(90px);opacity:.07;pointer-events:none;"></div>
      <div style="position:absolute;width:350px;height:350px;border-radius:50%;bottom:-60px;right:10%;background:radial-gradient(circle,var(--accent) 0%,transparent 65%);filter:blur(75px);opacity:.06;pointer-events:none;"></div>
      <div style="width:100%;padding:0 clamp(24px,6vw,96px);">
        <div class="about-layout" style="max-width:1100px;margin:0 auto;display:grid;grid-template-columns:1fr 1fr;
                    gap:clamp(48px,6vw,96px);align-items:center;">
          <div>
            <p class="label" style="margin-bottom:16px;">02 — ABOUT ME</p>
            <h2 style="font-family:'Playfair Display',serif;font-size:clamp(2.5rem,6vw,5rem);
                       font-weight:600;line-height:1.1;margin-bottom:28px;color:var(--fg);">
              Crafting<br><em style="color:var(--accent);">experiences</em><br>that matter.
            </h2>
            <p style="color:var(--fg2);line-height:1.85;max-width:420px;margin-bottom:28px;">
            ...
            <a href="/about" class="label"
              ...
              on:click|preventDefault={() => { try { sessionStorage.setItem('fromSection', String(activeIdx)); } catch(e) {} goto('/about'); }}>
              <span style="display:block;width:40px;height:2px;background:var(--accent);"></span>
              LEARN MORE →
            </a>
          </div>
          <div class="stats-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
            {#each [{n:'5+',l:'Years Experience'},{n:'30+',l:'Projects Delivered'},{n:'12+',l:'Happy Clients'},{n:'∞',l:'Cups of Coffee'}] as s}
              <div class="glow-card"
                ...
                <p style="font-family:'Playfair Display',serif;font-size:2.5rem;font-weight:600;color:var(--accent);margin-bottom:4px;">{s.n}</p>
    ...
    <!-- ══ SECTION 3 · CONTACT ══ -->
    <section class="fp-section" style="background:var(--bg);">
      <div class="float" style="position:absolute;width:500px;height:500px;border-radius:50%;
        left:-100px;bottom:-100px;background:radial-gradient(circle,var(--accent) 0%,transparent 70%);
        filter:blur(80px);opacity:.12;pointer-events:none;"></div>
      <div style="position:relative;z-index:10;width:100%;padding:0 clamp(24px,6vw,96px);text-align:center;">
        <p class="label" style="margin-bottom:16px;">03 — CONTACT</p>
        <h2 style="font-family:'Playfair Display',serif;font-size:clamp(3rem,8vw,7rem);
                   font-weight:600;line-height:1.05;margin-bottom:28px;color:var(--fg);">
          Let's create<br>something <em style="color:var(--accent);">great.</em>
        </h2>
        <p style="color:var(--fg2);font-size:1.1rem;margin-bottom:40px;">Have a project in mind? I'd love to hear about it.</p>
        <a href="/contact" class="label cta-btn"
          ...
          on:click|preventDefault={() => { try { sessionStorage.setItem('fromSection', String(activeIdx)); } catch(e) {} goto('/contact'); }}>
          GET IN TOUCH <span>→</span>
        </a>
        <div style="display:flex;justify-content:center;gap:32px;margin-top:56px;">
          {#each [{name:'TWITTER', url:'https://twitter.com'},{name:'GITHUB', url:'https://github.com'},{name:'DRIBBBLE', url:'https://dribbble.com'}] as s}
            <a href="{s.url}" class="label"
...
<style>
  ...
  .fp-section { height:100vh;width:100%;position:relative;overflow:hidden;display:flex;align-items:center; }
  .work-row-btn:hover .row-title { color:var(--accent); }
  .work-row-btn:hover .row-arrow { transform:translateX(8px); }
  .work-row-btn:hover .hover-line { width:100% !important; }
  .work-row-btn:hover .card-thumb { transform:scale(1.05); }
  .cta-btn:hover { background:var(--accent) !important; color:var(--bg) !important; }
  .glow-card { transition:border-color .35s,box-shadow .35s; }
  .glow-card:hover { border-color:var(--accent) !important; box-shadow:0 0 18px -4px var(--accent),0 0 40px -12px var(--accent); }
  @keyframes float { 0%,100%{transform:translateY(0);} 50%{transform:translateY(-14px);} }
  .float { animation:float 7s ease-in-out infinite; }
  @media(max-width:640px) { .fp-section{align-items:flex-start !important;padding-top:80px !important;} }
  @media(max-width:768px) {
    .about-layout { grid-template-columns: 1fr !important; gap: clamp(32px,6vw,48px) !important; }
    ...
    .hero-svg-wrap svg { width: 80% !important; max-width: 320px !important; }
  }
  @media(max-width:480px) {
    .stats-grid { grid-template-columns: 1fr !important; }
    .glow-card { padding: 18px !important; }
  }
</style>
