<script>
  ...
  async function goBack() {
    ...
    if (sourceRow) {
      sourceRow.querySelectorAll("*").forEach((el) => {
        const cs = window.getComputedStyle(el);
        liveFontMap.set(el, { fontFamily: cs.fontFamily, fontSize: cs.fontSize });
      });
    }
...
>
  ...
  >
    ...
    >
      <div style="display:flex;align-items:center;gap:var(--work-row-gap);">
        <span
          class="label"
          style="width:32px;flex-shrink:0;color:rgba(255,255,255,.45);">03</span
        >
        <div
    ...
    <div
      ...
      style="position:absolute;width:700px;height:700px;border-radius:50%;top:-200px;right:-60px;background:radial-gradient(circle,#4ade80 0%,transparent 65%);filter:blur(90px);opacity:0;pointer-events:none;transition:opacity 2.0s ease;"
    ></div>
    <div
      class="hero-glow hero-glow-2"
      style="position:absolute;width:500px;height:500px;border-radius:50%;bottom:-100px;left:-80px;background:radial-gradient(circle,#4ade80 0%,transparent 65%);filter:blur(80px);opacity:0;pointer-events:none;transition:opacity 2.4s ease 0.5s;"
    ></div>
    <div
      class="hero-glow hero-glow-3"
      style="position:absolute;width:300px;height:300px;border-radius:50%;top:35%;right:28%;background:radial-gradient(circle,#4ade80 0%,transparent 65%);filter:blur(65px);opacity:0;pointer-events:none;transition:opacity 2.2s ease 1.0s;"
    ></div>
  ...
  >
    <div class="reveal" style="margin-bottom:48px;">
      <p class="label" style="margin-bottom:14px;color:#4ade80;">
      ...
      {#each Array(6) as _, i}
        ...
        >
          ...
          >
            <div>
              <div
                style="height:6px;width:55%;border-radius:2px;margin-bottom:5px;background:#4ade80;opacity:.3;"
              ></div>
              <div
