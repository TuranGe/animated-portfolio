<script>
  ...
  const HERO_BG    = '#0d2010';
  const ACCENT_CLR = '#4ade80';
...
<svelte:head><title>Mori Gardens — Portfolio</title></svelte:head>
<Nav bind:this={navEl} {sections} subPage={true} showSidebar={false} onBack={goBack} onNavigate={() => goBack()} />

<div bind:this={pageEl} class="page-scroll" style="position:fixed;inset:0;background:var(--bg);overflow-y:auto;z-index:100;">

  <div bind:this={heroEl}
    style="width:100%;height:100vh;position:relative;overflow:hidden;
           display:flex;align-items:center;flex-shrink:0;">

    <div class="hero-row" style="width:100%;padding:0 clamp(24px,6vw,96px);box-sizing:border-box;">
      <div style="display:flex;align-items:center;gap:20px;">
        <span class="label" style="width:32px;flex-shrink:0;color:rgba(255,255,255,.45);">03</span>
        <div style="width:96px;height:64px;border-radius:6px;overflow:hidden;flex-shrink:0;
                    background:#0d2010;display:flex;align-items:center;justify-content:center;">
          <span style="font-size:1.4rem;opacity:.8;color:#4ade80;">◆</span>
        </div>
        <div style="flex:1;">
          <h1 style="font-family:'Playfair Display',serif;font-size:clamp(1.3rem,3vw,2.4rem);
                     font-weight:600;color:rgba(255,255,255,.92);margin-bottom:4px;">Mori Gardens</h1>
          <p class="label" style="color:rgba(255,255,255,.4);">Web Design / Motion — 2023</p>
        </div>
    ...
    <div class="hero-glow hero-glow-1" style="position:absolute;width:700px;height:700px;border-radius:50%;top:-200px;right:-60px;background:radial-gradient(circle,#4ade80 0%,transparent 65%);filter:blur(90px);opacity:0;pointer-events:none;transition:opacity 2.0s ease;"></div>
    <div class="hero-glow hero-glow-2" style="position:absolute;width:500px;height:500px;border-radius:50%;bottom:-100px;left:-80px;background:radial-gradient(circle,#4ade80 0%,transparent 65%);filter:blur(80px);opacity:0;pointer-events:none;transition:opacity 2.4s ease 0.5s;"></div>
    <div class="hero-glow hero-glow-3" style="position:absolute;width:300px;height:300px;border-radius:50%;top:35%;right:28%;background:radial-gradient(circle,#4ade80 0%,transparent 65%);filter:blur(65px);opacity:0;pointer-events:none;transition:opacity 2.2s ease 1.0s;"></div>
    
    <div class="hero-fade" style="position:absolute;bottom:0;left:0;right:0;height:200px;
  ...
  <div class="page-body" style="padding:80px clamp(24px,6vw,96px) 120px;max-width:1100px;margin:0 auto;">
    <div class="reveal" style="margin-bottom:48px;">
      <p class="label" style="margin-bottom:14px;color:#4ade80;">03 — Web Design / Motion</p>
      <h2 style="font-family:'Playfair Display',serif;font-size:clamp(3rem,9vw,8rem);
                 font-weight:600;line-height:.95;color:var(--fg);margin-bottom:40px;">Mori Gardens</h2>
      <p style="color:var(--fg2);max-width:620px;line-height:1.85;font-size:1.05rem;">
        A carefully crafted digital experience pushing the boundaries of interaction and visual storytelling.
      </p>
    </div>
    <div class="reveal" style="display:grid;grid-template-columns:1fr 1fr;gap:48px;margin-bottom:80px;">
      <div>
        ...
        <p style="color:var(--fg2);line-height:1.85;">A carefully crafted digital experience for a forward-thinking client, exploring new interaction paradigms while maintaining visual coherence.</p>
      </div>
      <div>
        <p class="label" style="margin-bottom:16px;">OUR APPROACH</p>
        <p style="color:var(--fg2);line-height:1.85;">Purposeful motion, typographic hierarchy, and deep respect for the user's attention and time guided every decision.</p>
      </div>
    </div>
    <div class="reveal" style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:80px;">
      {#each Array(6) as _, i}
        <div style="aspect-ratio:4/3;border-radius:6px;border:1px solid var(--border);
          ...
          <div style="width:100%;height:100%;display:flex;align-items:flex-end;padding:14px;">
            <div>
              <div style="height:6px;width:55%;border-radius:2px;margin-bottom:5px;background:#4ade80;opacity:.3;"></div>
              <div style="height:5px;width:38%;border-radius:2px;background:rgba(255,255,255,.06);"></div>
            </div>
          </div>
        </div>
      {/each}
    ...
    <div class="reveal" style="display:flex;justify-content:space-between;align-items:center;
      ...
      <a href="/work/arc" class="label"
        on:click|preventDefault={() => goNext('/work/arc')}
