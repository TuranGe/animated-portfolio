<script>
  ...
  const HERO_BG    = '#1a0f2e';
  const ACCENT_CLR = '#c084fc';
  ...
  onMount(async () => {
    ...
    revealEls.forEach((el, i) => {
      // Last reveal (Next Project row) uses 'top bottom' so it always fires
...
<svelte:head><title>Arc Architecture — Portfolio</title></svelte:head>
...
<div bind:this={pageEl} class="page-scroll" style="position:fixed;inset:0;background:var(--bg);overflow-y:auto;z-index:100;">
  ...
  <div bind:this={heroEl}
    ...
    <div class="hero-row" style="width:100%;padding:0 clamp(24px,6vw,96px);box-sizing:border-box;">
      <div style="display:flex;align-items:center;gap:20px;">
        <span class="label" style="width:32px;flex-shrink:0;color:rgba(255,255,255,.45);">04</span>
        <div style="width:96px;height:64px;border-radius:6px;overflow:hidden;flex-shrink:0;
                    background:#1a0f2e;display:flex;align-items:center;justify-content:center;">
        ...
        <div style="flex:1;">
          <h1 style="font-family:'Playfair Display',serif;font-size:clamp(1.3rem,3vw,2.4rem);
                     font-weight:600;color:rgba(255,255,255,.92);margin-bottom:4px;">Arc Architecture</h1>
          <p class="label" style="color:rgba(255,255,255,.4);">Digital Presence — 2023</p>
        </div>
    ...
    <div class="hero-glow hero-glow-1" style="position:absolute;width:700px;height:700px;border-radius:50%;top:-200px;right:-60px;background:radial-gradient(circle,#c084fc 0%,transparent 65%);filter:blur(90px);opacity:0;pointer-events:none;transition:opacity 2.0s ease;"></div>
    <div class="hero-glow hero-glow-2" style="position:absolute;width:500px;height:500px;border-radius:50%;bottom:-100px;left:-80px;background:radial-gradient(circle,#c084fc 0%,transparent 65%);filter:blur(80px);opacity:0;pointer-events:none;transition:opacity 2.4s ease 0.5s;"></div>
    <div class="hero-glow hero-glow-3" style="position:absolute;width:300px;height:300px;border-radius:50%;top:35%;right:28%;background:radial-gradient(circle,#c084fc 0%,transparent 65%);filter:blur(65px);opacity:0;pointer-events:none;transition:opacity 2.2s ease 1.0s;"></div>
  ...
  <div class="page-body" style="padding:80px clamp(24px,6vw,96px) 120px;max-width:1100px;margin:0 auto;">
    <div class="reveal" style="margin-bottom:48px;">
      <p class="label" style="margin-bottom:14px;color:#c084fc;">04 — Digital Presence</p>
      <h2 style="font-family:'Playfair Display',serif;font-size:clamp(3rem,9vw,8rem);
                 font-weight:600;line-height:.95;color:var(--fg);margin-bottom:40px;">Arc Architecture</h2>
      ...
      {#each Array(6) as _, i}
        <div style="aspect-ratio:4/3;border-radius:6px;border:1px solid var(--border);
          ...
          <div style="width:100%;height:100%;display:flex;align-items:flex-end;padding:14px;">
            <div>
              <div style="height:6px;width:55%;border-radius:2px;margin-bottom:5px;background:#c084fc;opacity:.3;"></div>
