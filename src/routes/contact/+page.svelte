<script>
  ...
  let leaving = false;
  let name = '', email = '', message = '', sending = false, submitted = false;
  ...
  async function goBack() {
    ...
    try { const from = sessionStorage.getItem('fromSection'); sessionStorage.removeItem('fromSection'); sessionStorage.setItem('returnSection', from !== null ? from : '3'); } catch(e) {}
    goto('/');
  }
  ...
  function handleSubmit() {
    ...
    sending = true;
    setTimeout(() => { sending = false; submitted = true; }, 1200);
  }
  ...
  onMount(async () => {
    ...
    gsap.fromTo('.ct-el',
      { y: 30, opacity: 0 },
      { y: 0, opacity: 1, duration: 0.8, ease: 'expo.out', stagger: 0.09, delay: 0.3 }
    );
  });
</script>
...
<!-- Ambient glows -->
<div style="position:fixed;width:600px;height:600px;border-radius:50%;bottom:-150px;left:-100px;background:radial-gradient(circle,var(--accent) 0%,transparent 65%);filter:blur(100px);opacity:.09;pointer-events:none;z-index:99;"></div>
<div style="position:fixed;width:350px;height:350px;border-radius:50%;top:-80px;right:-60px;background:radial-gradient(circle,var(--accent) 0%,transparent 65%);filter:blur(80px);opacity:.07;pointer-events:none;z-index:99;"></div>
...
<div bind:this={pageEl} class="page-scroll"
  ...
  <div style="width:100%;padding:0 clamp(24px,6vw,96px);max-width:1100px;margin:0 auto;">
    <div class="contact-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:clamp(48px,8vw,96px);">

      <!-- Left -->
      <div>
        <p class="ct-el label" style="margin-bottom:12px;opacity:0;">03 — CONTACT</p>
        <h1 class="ct-el"
        ...
        <p class="ct-el" style="color:var(--fg2);line-height:1.85;max-width:360px;margin-bottom:36px;opacity:0;">
          Have a project in mind, or just want to say hello? Drop me a message — I'll respond within 24 hours.
        </p>
        ...
        <div class="ct-el" style="margin-bottom:32px;opacity:0;">
          ...
          <a href="mailto:hello@portfolio.dev"
            ...
            onmouseenter="this.style.color='var(--accent)'" onmouseleave="this.style.color='var(--fg)'">
            hello@portfolio.dev
          </a>
        </div>

        <div class="ct-el" style="margin-bottom:32px;opacity:0;">
          <p class="label" style="color:var(--fg2);margin-bottom:6px;">AVAILABILITY</p>
        ...
        <div class="ct-el" style="display:flex;gap:24px;opacity:0;">
          {#each [{name:'TWITTER', url:'https://twitter.com'}, {name:'GITHUB', url:'https://github.com'}, {name:'LINKEDIN', url:'https://linkedin.com'}] as s}
            <a href="{s.url}" class="label"
              ...
              onmouseleave="this.style.opacity=.6;this.style.borderBottomColor='transparent'">{s.name}</a>
          {/each}
        </div>
      </div>
      ...
      <div class="ct-el" style="opacity:0;">
        {#if submitted}
          <div style="display:flex;flex-direction:column;align-items:flex-start;gap:20px;padding:48px 0;">
            <div style="width:44px;height:44px;display:flex;align-items:center;justify-content:center;
                        font-size:1.1rem;color:var(--accent);border:1.5px solid var(--accent);">✓</div>
            <h2 style="font-family:'Playfair Display',serif;font-size:2rem;color:var(--fg);">Message sent!</h2>
            <p style="color:var(--fg2);">Thanks for reaching out. I'll be in touch soon.</p>
            <button on:click={() => { submitted=false; name=''; email=''; message=''; }}
              class="label" style="color:var(--accent);background:none;border:none;cursor:pointer;transition:opacity .2s;">
              SEND ANOTHER →
            </button>
          </div>
        {:else}
          <div style="display:flex;flex-direction:column;gap:28px;">
            ...
            <div>
              <label for="name" class="label" style="display:block;margin-bottom:8px;color:var(--fg2);">YOUR NAME</label>
              <input
                id="name"
                bind:value={name}
                type="text"
            ...
            <div>
              <label for="email" class="label" style="display:block;margin-bottom:8px;color:var(--fg2);">EMAIL ADDRESS</label>
              <input
                id="email"
                bind:value={email}
                type="email"
            ...
            <!-- Message textarea -->
            <div>
              <label for="message" class="label" style="display:block;margin-bottom:8px;color:var(--fg2);">YOUR MESSAGE</label>
              <textarea
                id="message"
                bind:value={message}
                rows="5"
            ...
            >
              {#if sending}
                <span style="width:14px;height:14px;border-radius:50%;border:1.5px solid currentColor;
                             border-top-color:transparent;display:inline-block;animation:spin .7s linear infinite;"></span>
                SENDING...
              {:else}
                SEND MESSAGE →
              {/if}
...
<style>
  ...
  input::placeholder, textarea::placeholder { color:var(--fg2); opacity:.4; }
  @keyframes spin { to { transform:rotate(360deg); } }
  .contact-grid { width: 100%; box-sizing: border-box; }
  @media(max-width:768px) {
    .contact-grid {
      grid-template-columns: 1fr !important;
      gap: 40px !important;
    }
    .page-scroll { align-items: flex-start; }
  }
